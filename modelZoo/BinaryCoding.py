...
class contrastiveNet(nn.Module):
    ...
    def forward(self, x, bi_thresh, nClip=4):
        ...
        if self.fineTune == False:
            ...
            embedding1 = embedding1.reshape(bz, -1, embedding1.shape[-1])
            embedding2 = embedding2.reshape(bz, -1, embedding2.shape[-1])

            embed1 = torch.mean(embedding1, 1)
            embed2 = torch.mean(embedding2, 1)
