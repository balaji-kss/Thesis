...
class BaseGroupLasso(ABC, BaseEstimator, TransformerMixin):
    ...
    def _minimise_loss(self):
        ...
        optimiser = FISTAProblem(
            ...
            self._regulariser, # l1+l2 regulariser term
            self.subsampler_.subsample_apply(
                self._unregularised_gradient, self.X_aug_, self.y_  # gradient of reconstruction loss
            ),
            self._scaled_prox,  # l1,l2 proximal
            self.lipschitz_,
        )
...
if __name__ == "__main__":
    ...
    groups = [1, 1, 1, 2, 2, 3, 3, -1]


    group_lasso = BaseGroupLasso(groups=groups,
        group_reg=0.05,
        l1_reg=0.01,
        n_iter=100,
        tol=1e-5,
    ...
    D = torch.randn(36, 161)
    Y = torch.randn(1,36,8)

    coeff = group_lasso.fit(D, Y, lipschitz=None)
